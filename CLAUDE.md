...
rule is in `~/.claude/CLAUDE.md`. Launch batch agents only for the next ship or two. Do
...
- **Captions say exactly what the image is and when it was made (owner rule, 2026-09-15).** Never hedge between periods ("a medieval or later depiction", "a historical depiction", "an associated visual source", "image associated with X"). A caption names the medium and the date or century, for example "a thirteenth-century manuscript illumination", "a 1837 history painting by …" or "a 2012 photograph". If the date is unknown, it says so specifically ("undated nineteenth-century engraving"), never "medieval or later". Reported on `baldwin-ii-of-jerusalem`, whose caption read "Baldwin II of Jerusalem in a medieval or later historical depiction". A placeholder caption usually means nobody checked the image either: the same template on `louis-ix-of-france` sat under a photograph of a nineteenth-century church. So when fixing one, view the image as well as rewriting the words.
- **Colour, and the subject large in the frame (owner rule, 2026-09-15).** Avoid black-and-white images unless there is genuinely no other option, and never use a main image where the subject is small, distant or hard to make out. Reported on `kerak`: its main image was a black-and-white photograph in which the castle ruins were far away and barely visible. This refines the "broad pre-modern view" rule for cities below. A colour lithograph, painting or photochrom meets both rules. For a castle, monument or ruin, a sharp colour photograph in which the medieval structure fills the frame beats a distant historic view, whatever the view's age.
- **No margins around images, whatsoever (owner rule, 2026-09-15).** An article image fills its frame edge to edge: no padding, tile or border added by the CSS, and no source file with a wide blank or white border baked in. Reported on `teutonic-order`, whose photograph sat inside a cream tile with 40px of padding (styling left over from when orders led with cross emblems), and on the other order pages linked in the same test case. **How to apply:** image selectors never set `padding` (the dark `#111116` letterbox from `object-fit: contain` is the only allowed surround, for images whose shape differs from the frame). When choosing a file, reject one with a blank border, or crop the border off. **A plain white background is not a margin (owner decision, same day).** An object photographed or illustrated on a plain white ground keeps that ground, even when it is a long, thin pike or lance and most of the frame is white. The same goes for coins and seals on a white or paper ground: the owner looked at tight-crop and dark-ground versions and kept them as they are. The rule targets a thick blank border around a picture: the paper around a print or a manuscript page, a card mount, a scan's page edge. `scripts/audit-image-borders.mjs` flags both kinds, so Weapons & Armor objects on white come up in its report and are left alone; `scripts/crop-image-borders.mjs` crops the real borders. Where the owner asks for a wide view of a building, prefer a shot in which the whole complex fills the frame, such as a drone photograph (the Malbork drone shot on `teutonic-order`), over a detail or an interior.
...
- **When no depiction of any kind survives, lead with a documented associated monument (owner decision, 2026-09-15).** The search comes first and is not optional: a manuscript image, effigy, seal, portrait coin or later artwork, captioned honestly, always beats this. Only when none exists may the main image be a monument or place documented as the person's own — a foundation, a tomb, a building they commissioned, a royal seat — and the caption's **first sentence** must say plainly that it is not a likeness, e.g. "No likeness of Ealhswith survives; this is the site of the Nunnaminster, the abbey she founded at Winchester." The `note` records what was searched. This settles the five rulers who led with aniconic coins (`al-adil-ii`, `al-mansur-ali`, `baraka-khan`, `yusuf-ibn-tashfin`, `muhammad-al-nasir`): their coins move to section images and a documented monument leads. It is not a licence to skip the search, and a faceless coin or a map still never leads. **A landscape or bare site photograph never leads a person either (owner rule, 2026-09-15, on `harald-greycloak`: "Anything is better than a random landscape").** Where no likeness exists, prefer anything that depicts the person, however symbolic, over a place: a later illustration of a scene they are in, such as a saga illustration showing them among others, a coin, or a seal. A monument qualifies only as a built structure documented as theirs (a tomb, a foundation), never as the scenery where something happened.
...
- Damascus must not use a generic modern city image. Since 2026-09-15 it leads with Félix Bonfils's photograph of the old city of about 1870 (see the next rule), with the Umayyad Mosque and the Ayyubid Citadel as section images.
- **City main images: a broad pre-modern view of the whole city (owner decision, 2026-09-15).** For major cities the owner wants a wide view of the city rather than a single monument, while a modern skyline stays banned. Satisfy both with a pre-modern depiction of the whole city: a medieval manuscript view, a woodcut or engraving (the Nuremberg Chronicle of 1493, Braun and Hogenberg's Civitates Orbis Terrarum), an early modern painting, or a nineteenth-century photograph taken before modern expansion. The caption says what it is and when it was made; the note says what in it post-dates the Middle Ages. The monument photograph that used to lead becomes a section image, so the city carries at least two images. Worked examples: Rome (Nuremberg Chronicle), Toledo (El Greco), Granada (Hoefnagel), London (Royal MS 16 F II), Damascus (Bonfils). A very wide panorama, such as Visscher's London at about 5:1, renders as a thin strip and belongs in a section instead. Captions must not use initials with full stops ("J. M. W."), which the two-sentence caption check counts as sentence breaks.
...
- **Every city carries a locator map in its Overview (owner rule, 2026-09-15).** A clear map of the surrounding region with a single marker and the place named beside it, so a reader sees at a glance where the city lies, in the style of a Wikipedia location map. Reported on `kerak` ("where in the Levant is Kerak"). It is additional to the main and section images, never a replacement for them. **The base map is always medieval if possible** (owner decision, same day). It shows the medieval polity or region the city belonged to, such as the Kingdom of Jerusalem for crusader-era Levant cities. A modern-borders base is a fallback, used only where no medieval base map is possible, and its caption then says the borders are modern. The build plan is QUEUE 0k: coordinates per city, plus one `LocatorMap` component over published location-map bases.
  **Layout standard (owner-approved 2026-09-15, on `kerak`):** the map never floats in part-way down the text. The section becomes two columns: the heading spans the top, all the text fills the left column from its first line, and the figure sits top-aligned in its own borderless right-hand column (`sideFigure` on `ArticleSection`, `.section-with-side-figure` in the CSS). Below 820px the columns stack, text first. Use the same layout for any future figure placed beside a section's text.
