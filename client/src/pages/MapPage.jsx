    ...
    <section className="content-section page-section map-page">
      ...
      */}
      <div className="map-hero">
        ...
        */}
        {/*
          "How to read this map", and the coverage inventory the brief's
          definition of done asks for. Closed by default — a reader who wants the
          map should not have to scroll past a disclaimer — but never hidden, and
          never softened. The numbers in it are generated by the build, so they
          cannot drift away from what is actually published.
        */}
        <details className="map-guide">
          <summary>How to read this map, and what it does not show</summary>

          <div className="map-guide-body">
            <h2>The year you choose is not the year of the evidence</h2>
            <p>
              The slider runs across the whole Codex period, 476 to 1453. The evidence does
              not. This map holds {SNAPSHOT_YEARS.length} dated reconstructions —{' '}
              {SNAPSHOT_YEARS.join(', ')} — and for any other year it shows the nearest one{' '}
              <strong>at or before</strong> it, and says so above. Never a later one: that would
              put states on the map before they existed, which is a fabricated claim rather than
              a rounding error.
            </p>

            <h2>Every frontier here is approximate</h2>
            <p>
              The source rates the precision of each border on a scale of one to three. Every
              feature in every year of this map is rated one, its lowest — "approximate". The
              dashed edges are a reminder of that, not a distinction between a firm border and
              a vague one. There are no firm borders on this map.
            </p>

            <h2>Blank ground is not empty land</h2>
            <p>
              Where no polity is drawn, nobody has mapped who held that ground — not that it was
              empty, unsettled or unruled. The physical coastline underneath is there so the
              difference between sea and unmapped land is visible at a glance.
              {coverage && (
                <> Before {coverage.unmappedBefore} there is no source at all, so the map is blank.</>
              )}
            </p>

            <h2>The source contains errors, and they are kept</h2>
            <p>
              Known cases: at 1100 and 1200 Toledo is shown Almohad though Castile took it in
              1085; at 1100 Prague is shown Polish though Bohemia was imperial; at 1200 Ankara is
              shown Byzantine though Anatolia was Seljuk; at 1400 Athens is shown Byzantine
              though the Latin Duchy held it. These are inaccurate frontiers rather than states
              that did not exist, and redrawing a border by hand is the one thing this map must
              never do. Two labels that named states already dead —{' '}
              <em>Fatimid Caliphate</em> at 1200 and <em>Seljuk Caliphate</em> at 1400 — were
              removed, and that ground now reads as unmapped.
            </p>

            <h2>Not everything on the map has an article</h2>
            {coverage ? (
              <>
                <p>
                  {coverage.totals.unlinkedPolities} of the territories drawn here have no Codex
                  article yet. They are still drawn, named and selectable; the panel says plainly
                  that the article is missing rather than hiding the territory.
                </p>
                <table className="map-coverage">
                  <caption>Coverage by source year</caption>
                  <thead>
                    <tr>
                      <th scope="col">Year</th>
                      <th scope="col">Polities</th>
                      <th scope="col">With an article</th>
                    </tr>
                  </thead>
                  <tbody>
                    {coverage.years.map((row) => (
                      <tr key={row.year}>
                ...
                <p className="map-guide-note">
                  The early years are thin because the Codex has few articles on post-Roman
                  polities, not because the geometry is missing.
                </p>
              </>
            ) : (
              <p>The coverage inventory could not be loaded.</p>
            )}
          </div>
        </details>
