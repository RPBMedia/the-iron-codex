...
function normalizePerson(item) {
  ...
  return {
    ...
    aliases: [item.title, ...roles].filter(Boolean),
...
function normalizeEvent(item) {
  ...
  return {
    ...
    aliases: [item.eventType].filter(Boolean),
    tags: [item.eventType, item.location, ...(item.factions ?? [])].filter(Boolean),
    years,
    timelineYears: [],
    related: [...(item.leaders ?? []).map((leader) => leader.name), item.eventLocation?.name].filter(Boolean),
    searchableText: normalize([
      item.name,
      item.id,
      item.summary,
...
function normalizeLocation(item) {
  ...
  return {
    ...
    description: item.summary,
    dateLabel: String(item.year ?? ''),
    aliases: [item.kingdom].filter(Boolean),
