...
0s. **OWNER APPROVED 2026-09-16 — WRITE THEM. 8 of 14 done, 6 in flight.** Written: Alfonso X of Castile, Stephen I of Hungary, Ine of Wessex, Fernán González (`e92b71f`); Mindaugas, Gediminas, Algirdas, Kęstutis (`bf5180c`). In flight: the four Leonese rulers (Ordoño II, Alfonso V, Urraca, Alfonso IX) and the two Swedish regents (Engelbrekt Engelbrektsson, Sten Sture the Elder). The regents raise a real question the agent was told to decide and justify: a regent is not a monarch, so `isRuler` and a succession box may not belong — `birger-jarl` is the precedent to follow.
   **Original entry, for the record:**
0s-old. **OWNER DECISION NEEDED: 14 rulers are named on polity pages with no article behind them.** Found 2026-09-16 while fixing polity ruler coverage (`187b1c3` and the fix that followed). Each is dead text on a page that names them as a major ruler, which is the create-or-reconsider case CLAUDE.md describes: either write the article or stop calling them major.
...
0t. **SMALL AND CERTAIN: 4 house members are named without a link although their article exists.** Surveyed 2026-09-16, the house equivalent of the polity-ruler gap. `almohad-dynasty` → Yaqub al-Mansur, `house-of-lazarevic` → Milica, `house-of-courtenay` → Baldwin II (which resolves to `baldwin-ii-of-jerusalem` — **verify that is the same man before linking**, since the Courtenays produced their own Baldwins), `amorian-dynasty` → Theodora. Each is a `notableMembers` entry or a family-tree node missing its `personSlug`. Cheap, and the same one-way drift the ruler gate now catches for polities — worth a gate of its own once fixed, since `validateHouseTree` already walks these structures.

0u. **NEEDS JUDGEMENT, NOT A SWEEP: 85 person-place pairs where a birth or death place never mentions the person.** Surveyed 2026-09-16 across 25 places. **Do not fix this mechanically** — most of it is correct as it stands:
   - **39 are Constantinople**, which cannot list every emperor born or dead there without becoming a register. A city article names the people whose story happened *there*; a birth is often just a fact about the person.
   - **24 more are realms, not places** — `kingdom-of-england`, `kingdom-of-france`, `kingdom-of-denmark`, `holy-roman-empire` — where the "place" is a polity and the expectation makes no sense.
   - **The real candidates are the small, specific places**, where the person IS the reason the article exists: `madrigal-de-las-altas-torres` (Isabella I born), `kirkwall` (Haakon IV died), `oslo` (Haakon VI died), `ribe` (Christopher I died), `agincourt` (Charles d'Albret died). That is a handful of sentences, not a programme.
   - **`northumbria` → Ragnar Lothbrok is a trap**: his death there is legend, the archive says so, and naming him as fact would contradict our own article.
   **If this is ever gated**, the rule must exclude polity-type locations and cap by article size, or it will demand padding — which is what the no-filler rules forbid.
...
0k. **OWNER REQUEST 2026-09-15, a big one: a locator map in every city's Overview.**
   ...
   **STILL QUEUED — the owner wants them all eventually:** Iberia (9 places), Byzantium and Anatolia (6), Germany and the Empire (7), Italy (2), and the six outliers with coordinates but no frame (Tikrit, Manzikert, Grunwald, Marrakesh, Horodok, Novgorod). Each needs a bounds box or a calibratable map; the same `COUNTRY_TO_MAP` table extends to them.
