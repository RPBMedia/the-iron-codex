...
0s. **OWNER APPROVED 2026-09-16 — 13 of 14 written; ONE HELD for the owner.** Written: Alfonso X of Castile, Stephen I of Hungary, Ine of Wessex, Fernán González (`e92b71f`); Mindaugas, Gediminas, Algirdas, Kęstutis (`bf5180c`); Engelbrekt Engelbrektsson; and Ordoño II, Alfonso V, Urraca and Alfonso IX of León (`0613742`).
   **HELD — a scope call only the owner can make: Sten Sture the Elder.** He is written, all gates green, sitting in the session scratchpad one command from shipping. His regency begins in **1470**, and CLAUDE.md's scope rule says a rule beginning in 1454 or later is outside the Codex era *"whatever a task spec says"*, naming Ivan III (1462) and Hans (1481) as the same case. This item names him, so two owner instructions genuinely conflict and the call is not the assistant's to make. Nothing is broken while it waits: Sweden's Major rulers section still names him as plain text, a legitimate state for a ruler with no article, and `charles-viii-of-sweden`'s successor endpoint stays the `outside-scope` entry the rule requires.
   **The regent question was answered both ways, which is the right answer.** Engelbrekt is **not** a ruler: *rikshövitsman* was a war command created in an emergency, Eric of Pomerania stayed crowned king of all three kingdoms throughout, and Engelbrekt settled with him in October 1435 and took Örebro in fief from him. Sten Sture **is**: Sweden had no king at all from 1470, the regent exercised royal government, and the chain is documented on both sides. The `birger-jarl` precedent is not "regents get boxes" but "a named office of the realm, held as head of government, with a documented succession" — the same test producing different results, not an inconsistency.
