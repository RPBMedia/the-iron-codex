...
0j. **OWNER RULE 2026-09-17 — a locator shows the WHOLE country, never a section of it.** Reported on `toledo`, whose inset was a crop: provincial borders and rivers with no coastline and no national outline, so the dot could be seen but not placed. **The principle is already applied everywhere** — the fix was to the `LocatorMap` component, so all 100 locators switched from crop to whole map in one commit, and `cropWindow()` was deleted with a comment where it lived saying not to bring it back.
   **What actually remains is six places whose base map is not a map of their own country:**
   - **`novgorod` → the Novgorod Oblast map.** The clear failure: showing the whole oblast still does not say where in Russia the city is. The original objection to a full Russia map was that it *cropped* to ~45° of longitude — an objection that no longer applies now that nothing crops. **Switch it to a whole-Russia map**, accepting continental scale, exactly as Spain works.
   - **`damascus`, `sidon`, `kerak` → the Kingdom of Jerusalem map.** Deliberate, and it follows the earlier owner rule that the base map is medieval wherever possible. This new rule points the other way. **Needs an owner ruling:** the medieval polity, or modern Syria, Lebanon and Jordan.
   - **`kosovo-field`, `prilepac` → "the central Balkans".** Already a whole-country map (Serbia's), titled regionally on purpose so a battlefield does not assert a contested border. Leave it.
   **Standing rule for any base map added from now on:** it must show a whole country, or a whole medieval polity, at its full extent. Never a window onto part of one. If a map is too tall for its column, cap the height in CSS and let the width shrink with it — `--locator-ar` already does this for Portugal, Sweden and the Kingdom of Jerusalem. A test asserts every marker lands inside its map by proportion.
...
0o. **OWNER REQUEST 2026-09-15: GLOBAL ARTICLE UI/UX REFINEMENT. A major track that will take a while.** The full spec is **Appendix B** at the end of this file in the repo root (42 sections). Read all of it before any milestone. The benchmark page is `/events/battle-of-brunanburh`. The goal is to move article pages from a database-looking layout toward a premium digital codex, **fixing shared components rather than single pages, and without redesigning the site's identity**: near-black, ivory, restrained gold, monumental titles, and the side-by-side image and title hero.
   ...
   **Milestones, one ship each, each with a test case:**
   ...
   - **U4: responsive** (wide desktop, laptop, tablet, mobile). ✅ **The breakpoint consolidation this milestone opened with is DONE — measured 2026-09-17.** The stylesheet holds **13 `@media` blocks and every one is on 1024, 860 or 640**, plus a single `prefers-reduced-motion` block. **The old "twelve distinct breakpoints" count was an artefact of the measurement:** it grepped `max-width`, which cannot tell a breakpoint from an ordinary property, and 760, 960 and 620 are element widths (`.section-heading.wide` and `.topic-intro` at 760px, `.wa-type-figure img` at 960px, `.insights-note` at 620px). Do not re-consolidate; there is nothing left to fold in.
     **What remains of U4 is the visual sweep**, which needs a live page at each tier and so is the owner's to verify, not something a static check can settle.
   - ✅ **U5: accessibility — DONE 2026-09-17.** Three `aria-label`s were doing nothing, because ARIA does not expose one on a generic element with no role. The house family tree's marriage symbol (`span.tree-marriage`, U+26AD) announced **nothing** on every house page and now carries `role="img"` — the only one of the three that cost a reader information. The `article-actions` wrapper and the favorites toolbar had their labels **removed** instead: each wrapped a single self-labelling control (a `FavoriteButton` with its own `aria-label`/`aria-pressed`, and a `select` named by its visible label), so a group of one was noise, not structure.
     **The rest of the surface audited clean — do not redo it.** Every `img` has `alt`: descriptive where it carries meaning, empty on the two decorative ones, with the locator map's wrapper holding `role="img"` so it is not announced twice. Every icon-only button already has an accessible name, and the menu trigger carries `aria-expanded` and `aria-controls`. The two `h1` tags are mutually exclusive branches (not-found state and article), not a duplicate. `ZoomableImage` closes on Escape, locks body scroll, sets `aria-modal` and `autoFocus`, and gives its zoom trigger `role="button"`, `tabIndex` and Enter/Space handling.
     **Method note:** a `grep` for `<img` without `alt=` on the same line proves nothing, because JSX attributes span lines — every one of the six it flagged turned out to be correct on inspection. The same trap applies to `onClick`.
   - **U6: regression** over the §34 page set at desktop and mobile widths.
   - ✅ **U7: cleanup of obsolete styles — DONE, verified 2026-09-17.** `styles.css` holds **249 class selectors and every one is referenced** in `client/src`. The six targeted earlier are gone: `.collection-tools`, `.inline-list`, `.sort-control` and `.wa-specs` without a trace, while `.reliability-note` and `.standfirst` leave only comments recording why they went — the first because `HistoricalReliabilityNote` was removed permanently, the second because the deck moved into the hero.
     **Any future prune must stay report-only unless it honours the deny-list.** Seven classes are built by interpolation — `detail-media-${article.type}`, `favorite-${variant}`, `tree-person-${variant}` — so they never appear literally in source. A naive "not found in source" sweep flags them, and deleting `.detail-media-weaponArmor` would break the Weapons & Armor full-object render mode **with every gate still green**: `check:render` asserts the class is in the markup, which it would be, while the styling that gives it meaning had gone.
...
0q. **BARE-NAMED BATTLES — 45 written; about 16 distinct battles still open.**
   ...
   **Related and unfixed: `validateBattleContinuity` has a blind spot of its own.** Its skip-check matches on an exact `conflict` string, so a link that steps over a new battle in a differently-named conflict stays green. That let Hastings keep pointing at Bouvines across Tinchebrai, and Durbe at Grunwald across Strėva and Rudau — **both caught by hand on 2026-09-17, neither by the gate.** Consider matching on year-range and region as well as the exact string.
   ...
   3. **No gate watches this class at all.** Succession endpoints have `validateStaleSuccessionEndpoints`; houses got their check on 2026-09-17; battle leaders have neither, so a commander who gains an article stays plain text and nothing complains. Add the equivalent to `check-content-quality.mjs`, with the same ambiguity-denylist discipline — a wrong leader link is worse than a missing one, and "Baldwin of Flanders" resolving to the Latin Emperor is exactly the shape that needs checking rather than auto-applying.
   Reciprocity is missing too: `sten-sture-the-elder` has an empty `relatedEntries.events` and should carry Brunkeberg, which he won.

   ⚠️ **OWNER REPORT 2026-09-17 — the Battle on the Ice (1242) has no article, and NO GATE COULD HAVE CAUGHT IT.** Reported on `alexander-nevsky`, whose prose names it four times, whose timeline carries an entry titled "The Battle on the Ice" with an empty `links` array, and whose `relatedEntries.events` is **empty** — so the commander-to-battle reciprocity rule had nothing to bite on either. No article exists under `battle-on-the-ice`, `battle-of-the-ice` or `battle-of-lake-peipus`.
   **Why the gates missed it, which is the transferable part.** `battlePhraseRe` matches `(?:Battle|Siege) of X` and nothing else, so a battle named "Battle **on the** Ice" is invisible to `validateBattleLinking`; and `barePlaceBattleRe` needs the `at Place (year)` shape, which the prose never uses. **This is the Falkirk hole again from a different direction:** not a bare name, but a name that is not of the form "Battle of X". Nothing on `BATTLE_BACKLOG` covers it, because nothing ever surfaced it.
   **Do these three in order:**
   1. **Write the battle.** Lake Peipus, 5 April 1242, Alexander Nevsky against the Livonian Order. Reciprocity both ways: the battle in Nevsky's `relatedEntries.events`, and a `links` entry on his existing timeline row. Watch the alias: bare "Ice" must never be minted, and `Lake Peipus` is a place as well as a battle, so check it against the collision test before writing. Related existing articles: `alexander-nevsky`, `livonian-order`, `teutonic-order`, `novgorod`, `kievan-rus`, `battle-of-saule` and `battle-of-durbe` for the Baltic sequence.
   2. **Widen the phrase regex** to catch `Battle on/at/upon the X`, and consider `Fall of X`, `Sack of X` and `Rout of X` while there. The archive already holds a `fall-of-granada` and a `fall-of-constantinople`, so the "of" forms beyond Battle/Siege are live too.
   3. **Sweep before assuming this is the only one.** Grep every article for capitalised phrases beginning "Battle " that the regex does not match, and for "the battle of the ..." lower-case forms. One report from the owner found this; a sweep should find the rest rather than waiting for the next report.
   ...
   **Still open:**
   ...
   - **The 17 surfaced by the widened bare-name gate on 2026-09-16 are now 11.** Written since: Lechfeld, Courtrai, Cortenuova, La Forbie, Mansurah (2026-09-16) and Dandanaqan (2026-09-17). Still open: Nechtansmere, Heavenfield, Velbazhd, Soissons, the Trent, the Zab, Ponza, Val-ès-Dunes, Bornhöved, Åsle, Épila. Each is a create-or-document decision, not a licence to leave them unwritten.
   - **The rest of the backlog, for whoever picks it up:** an Anglo-Saxon and Viking cluster (Aclea, Ashdown, Ellandun/Wroughton, Tettenhall, the Trent, Heavenfield, Nechtansmere), a Scandinavian one (Fimreite, Fotevik, Grathe Heath, Fýrisvellir, Hova, Sparrsätra, Åsle, Falköping, Visby, Largs, Bornhöved), an Iberian one (La Higueruela, Toro, Valverde, Alfarrobeira, Épila), a Baltic one (Saule, Rudau, Strėva, Blue Waters), and the Norman and Frankish pieces (Tinchebrai, Val-ès-Dunes, Lincoln, Tertry, Soissons, Fontenoy). **Blue Waters (1362) is the strongest single candidate**: the first serious reversal the Horde suffered in the Rus' lands, eighteen years before Kulikovo.
...
**SHIPPED 2026-09-20 — the coverage inventory and "How to read this map".** The
brief's definition of done asks for "a published inventory of which periods and
regions remain insufficiently mapped", and it is the one deliverable a
finished-looking interface cannot fake. `coverage.json` is generated by the build
(stats only, no geometry) and a test asserts every number in it against the
snapshots, so it cannot drift from what is published. On the page it is a
closed-by-default disclosure covering: the year is not the evidence date, every
frontier is approximate, blank ground is not empty land, the source's known
