...
0q. **BARE-NAMED BATTLES — 25 written; about 38 distinct battles still open.**
   ...
   **Count, measured 2026-09-17:** `BATTLE_BACKLOG` holds 44 strings, of which 4 are documented regex artifacts and two are spelling-variant pairs (ellandun/ellendun, tinchebrai/tinchebray) — so **about 38 distinct unwritten battles**. Two stale entries were removed the same day: `ethandun` (it is Edington, written long since, which now carries the alias) and `atoleiros` (written). **Nothing checks this list for entries that have gained articles**, unlike the arms backlog, so both sat unnoticed — worth a gate if the list is touched again. The owner found that the archive named Falkirk 14 times across 7 articles with no article behind it. The cause: `validateBattleLinking` only scans for the phrase "Battle of X", and the prose said "at Falkirk", "Falkirk (1298)", "Defeat at Falkirk". Not on `BATTLE_BACKLOG` either, so nothing was watching.
...
0k. **OWNER REQUEST 2026-09-15, a big one: a locator map in every city's Overview.**
   ...
   **These are modern outlines**, allowed as the documented fallback, and the caption says so. Crop windows land 140–450km across, which is the recognisable range — the objection to a single Europe-wide map does not apply at country scale.
   ✅ **ALL REGIONS SHIPPED 2026-09-17: 99 places carry a locator**, up from 50. Spain, Portugal, Germany, Italy and Anatolia closed the region list; Poland, Ukraine, Lithuania, Morocco, Iraq and a central Balkans base closed the outliers. Ten further places (Westminster, Winchester Cathedral, Scone, Jelling, Uppsala, Stiklestad, Helsingborg, Visingsö, Fitjar, Bømlo, Nykøbing Falster) turned out to sit in already-mapped countries and had simply never had `locatorMap` written — assignment now runs off `COUNTRY_TO_MAP`, which is what found them.
   **Two remain, and only one is work.** *Novgorod* needs a base map: the European Russia module is a projected map with formulas, not a degree box, and a full Russia map would crop to ~45° of longitude, which is a region rather than a locator. *Gestilren* is **permanently excluded, not a gap** — its own article says nobody has ever established where it was, and the Västergötland and Uppland cases are both set out there. Do not "fix" it by inventing coordinates.
