...
0p. **OWNER-APPROVED 2026-09-15: SCAN EVERY ARCHIVE IMAGE FOR BLANK BORDERS BAKED INTO THE FILE.** The CSS stopped padding images in `e3372f7` (owner rule: no margins around images, whatsoever), but a file can still carry its own margin. The Hélyot scan on `order-of-st-thomas-of-acre` had a wide page border; it is now cropped and self-hosted. `node scripts/audit-image-borders.mjs --out <file>` fetches every render image, flattens it over the dark frame and measures blank light edges. It reports three classes: `frame` (three or four sides), `pair` (two sides) and `single` (one side at 8% or more, often a plain sky). Steps:
   ...
   **Scan run 2026-09-15:** 1,109 images: 98 frame, 50 pair, 36 single, no errors. Reviewed by eye on contact sheets:
   ...
   - ✅ **Separate quality item — done, verified 2026-09-16.** All three were replaced during the batch-4 kingdom work: `vandal-kingdom` leads with the bold red-on-yellow extent map of about 455–477, `mongol-empire` with the four-khanate plate of 1290, `sasanian-empire` with the 2020 reconstruction of the empire in 620. Each also carries an arms panel. **0p is now closed entirely.**
...
0n. **OWNER REPORTS 2026-09-15 (evening), made while testing the away-mode ships:**
   1. ✅ **Done — verified 2026-09-16.** Known for runs through the auto-linker: `renderLinkedText(article.quickFacts?.knownFor, article)` in `PersonQuickFacts`, and `KnownForBlock` receives the article. The entry had gone stale. Shipping first: the W&A block, the location and polity list, and the person fact card all run through `renderLinkedText`, and CLAUDE.md §5 makes it a rule.
   2. ✅ **Fixed 2026-09-16: Insights content ran past the dark page area onto the light body.** The cause was in the SHARED rule, not the Insights page: `.page-section::before` paints the ground with an `auto` bottom and a fixed `min-height: clamp(660px, 62vw, 760px)`, so it is a band, not the page's height. Insights is the one page-section long enough to outrun it, and the ivory chart titles fell onto a light ground. `.insights-page::before` already carried `bottom: 0`, which could never work while the min-height kept forcing the floor; clearing it is the other half. Scoped to Insights, so Collection, Search, Index and Topics keep the band unchanged.
